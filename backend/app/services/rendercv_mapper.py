...
def map_to_rendercv_model(
    ...
    if header_info.get("linkedin"):
        social_networks.append({
            "network": "LinkedIn",
            "username": header_info.get("linkedin", "").split("linkedin.com/in/")[-1].replace("/", ""),
    ...
    if "leadership" in grouped_sections:
        sections_data["leadership"] = _build_experience(grouped_sections["leadership"]) # Re-use exp structure
...
def _build_skills(units: List[ScoredUnit]) -> List[Dict[str, Any]]:
    ...
    # De-duplicate
    unique = sorted(list(set(all_skills)))
    
    # Categorize
    categories = {
        "Languages": ["Python", "Java", "C++", "JavaScript", "TypeScript", "SQL", "Go", "Rust", "Swift", "Kotlin", "R", "MATLAB", "HTML", "CSS"],
        "Frameworks": ["React", "Node.js", "Flask", "Django", "FastAPI", "Spring", "Express", "Vue", "Angular", "Next.js", "Torch", "PyTorch", "TensorFlow"],
        "Tools": ["Git", "Docker", "Kubernetes", "AWS", "GCP", "Azure", "MongoDB", "PostgreSQL", "Redis", "Jira", "Linux", "Unix"]
    }
